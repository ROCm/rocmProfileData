...
drop view kernels_gpu8;
drop view kernels_gpu9;
...
create view kernels_gpu5 as select apiName, s2.string as kernelName, (o.end-o.start) as duration, o.gpuId, row_number() over () as numRow from rocpd_op o inner join rocpd_string s2 on s2.id = o.description_id inner join rocpd_string s3 on s3.id = o.opType_id inner join rocpd_api_ops rao on rao.op_id=o.id inner join api on api.id = rao.api_id  where s3.string like 'KernelExecution' and s2.string not like '%ccl%' and apiName not like 'hipLaunchKernel' and o.gpuId = 5 order by o.start;
...
create view kernels_gpu7 as select apiName, s2.string as kernelName, (o.end-o.start) as duration, o.gpuId, row_number() over () as numRow from rocpd_op o inner join rocpd_string s2 on s2.id = o.description_id inner join rocpd_string s3 on s3.id = o.opType_id inner join rocpd_api_ops rao on rao.op_id=o.id inner join api on api.id = rao.api_id  where s3.string like 'KernelExecution' and s2.string not like '%ccl%' and apiName not like 'hipLaunchKernel' and o.gpuId = 7 order by o.start;
create view kernels_gpu8 as select apiName, s2.string as kernelName, (o.end-o.start) as duration, o.gpuId, row_number() over () as numRow from rocpd_op o inner join rocpd_string s2 on s2.id = o.description_id inner join rocpd_string s3 on s3.id = o.opType_id inner join rocpd_api_ops rao on rao.op_id=o.id inner join api on api.id = rao.api_id  where s3.string like 'KernelExecution' and s2.string not like '%ccl%' and apiName not like 'hipLaunchKernel' and o.gpuId = 8 order by o.start;
create view kernels_gpu9 as select apiName, s2.string as kernelName, (o.end-o.start) as duration, o.gpuId, row_number() over () as numRow from rocpd_op o inner join rocpd_string s2 on s2.id = o.description_id inner join rocpd_string s3 on s3.id = o.opType_id inner join rocpd_api_ops rao on rao.op_id=o.id inner join api on api.id = rao.api_id  where s3.string like 'KernelExecution' and s2.string not like '%ccl%' and apiName not like 'hipLaunchKernel' and o.gpuId = 9 order by o.start;
...
select count(*) as numKernelsGPU8 from kernels_gpu8;
select count(*) as numKernelsGPU9 from kernels_gpu9;
